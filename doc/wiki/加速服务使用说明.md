...
首页提供三种模式切换，通过 `config.app.mode` 配置项控制，不同模式会影响拦截器、测速和梯子插件的启用状态。
...
- **IP测速：** 启用
- **彩蛋插件：** 关闭
- **适用场景：** 仅需 DNS 加速和 IP 测速功能，不需要中间人拦截，最安全
...
- **IP测速：** 启用
- **彩蛋插件：** 关闭
- **适用场景：** 日常使用，需要拦截器功能（SNI、代理、缓存、脚本注入等）

## 3.3. 增强模式（ow）

- **拦截器：** 启用
- **彩蛋插件：** 启用（overwall 插件）
- **适用场景：** 可使用的规则无法满足需求，同时需要简单的代理服务
...
| 缓存请求拦截器    | cacheXxx       | 104            | 如果缓存还生效，直接响应304，不发送该请求如果缓存已过期或无缓存，则发送请求  注：只对GET请求生效！             |
| 重定向拦截器      | redirect       | 105            | 重定向到指定地址，直接响应302，不发送该请求                                                                    |
| 请求篡改拦截器    | requestReplace | 111            | 篡改请求头，达到想要的目的                                                                                     |
| 请求重试拦截器    | retry          | 100            | 仅对`GET`、`HEAD`、`OPTIONS` 方法生效（可自定义）。收到 500 等错误状态码时自动重试请求，支持缓存小请求体 |
...
"example.com": {
  ".*":{
    "retry": {
      "times": 3,              // 最大重试次数
      "statuses": [500, 502, 503],  // 触发重试的状态码
      "methods": ["GET", "POST"]    // 触发重试的请求方法
    }
  }
}
```
...
**注意：** 缓存拦截器配置名前缀为 `cache`，后跟时间单位，实际配置名只有一个 `cacheRequest`，通过不同的时间单位字段触发缓存。支持的时间单位字段如下：

| 字段                                           | 单位 | 示例                     |
| ---------------------------------------------- | ---- | ------------------------ |
| `cacheSeconds` / `cacheMaxAge` / `cache` | 秒   | `"cacheSeconds": 3600` |
| `cacheMinutes`                               | 分钟 | `"cacheMinutes": 30`   |
| `cacheHours`                                 | 小时 | `"cacheHours": 2`      |
| `cacheDays`                                  | 天   | `"cacheDays": 365`     |
| `cacheWeeks`                                 | 周   | `"cacheWeeks": 2`      |
| `cacheMonths`                                | 月   | `"cacheMonths": 1`     |
| `cacheYears`                                 | 年   | `"cacheYears": 1`      |
...
```json
{
  "example.com": {
    ...
    "selected.ip.net" // 指定为域名，可以进行CNAME解析
  }
}
```
...
注意，最终指定所使用的IP优先级如下：
优先使用IP预设的IP，随后使用 **DNS设置** 功能所设置的DNS解析内容
